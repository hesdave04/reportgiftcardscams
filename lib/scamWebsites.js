...
export async function getSimilarDomains(row, limit = 10) {
  ...
  const sel = "domain, report_count, primary_scam_type, kind, trust_score, registrar, server_country";
...
export async function listDomains({ page = 0, pageSize = 50, tld, scamType, registrar, country, kind, orderBy = "report_count" } = {}) {
  ...
  let q = supabase.from("scam_domains").select("domain, report_count, organic_count, primary_scam_type, kind, trust_score, risk_level, registrar, server_country, latest_report, total_lost, tld", { count: "exact" });
  ...
  else q = q.eq("kind", "reported"); // legit + checked_only domains are not scam sites
...
export async function getDirectoryStats() {
  ...
  const [{ count: total }, { count: indexable }, { count: reports }] = await Promise.all([
    supabase.from("scam_domains").select("domain", { count: "exact", head: true }).eq("kind", "reported"),
